...
router.get('/doctor-requests', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    try {
...
router.post('/doctor-requests/:id/approve', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    const { id } = req.params;
    try {
        ...
        const hashedPassword = await bcryptjs_1.default.hash('trial123', 10);
...
router.post('/doctor-requests/:id/reject', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    const { id } = req.params;
    try {
        const request = await prisma.doctorrequest.findUnique({ where: { id } });
        if (!request) {
