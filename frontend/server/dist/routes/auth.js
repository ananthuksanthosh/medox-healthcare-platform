    ...
    try {
        ...
        const newUser = await prisma.user.create({
            data: {
                ...
                name,
                phone,
                role: client_1.user_role.PATIENT,
                patientprofile: {
                    create: {
...
// 4. UPDATE PROFILE
router.put('/update-profile', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'Unauthorized' });
...
// 5. UPDATE PASSWORD
router.put('/update-password', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'Unauthorized' });
...
// 6. DELETE ACCOUNT
router.delete('/delete-account', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'Unauthorized' });
    try {
