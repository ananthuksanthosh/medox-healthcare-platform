...
const SETTINGS_FILE_PATH = path_1.default.join(__dirname, '../../admin_settings.json');
// Helper to read setting file
function getSettings() {
    try {
        if (fs_1.default.existsSync(SETTINGS_FILE_PATH)) {
            const content = fs_1.default.readFileSync(SETTINGS_FILE_PATH, 'utf-8');
    ...
    return {
        ...
        profile: {
            name: 'Medora Administrator',
            email: 'admin@medora.com',
            phone: '+91 98765 43210',
            organisation: 'Medora Healthcare',
        }
    };
}
...
function saveSettings(settings) {
    try {
        fs_1.default.writeFileSync(SETTINGS_FILE_PATH, JSON.stringify(settings, null, 2), 'utf-8');
    }
    catch (err) {
...
router.get('/stats', auth_1.authenticateJWT, async (req, res) => {
    ...
    try {
        const totalPatients = await prisma.user.count({ where: { role: client_1.user_role.PATIENT } });
        const totalDoctors = await prisma.user.count({ where: { role: client_1.user_role.DOCTOR } });
        ...
        const completedAppointments = await prisma.appointment.count({ where: { status: client_1.appointment_status.COMPLETED } });
...
router.get('/appointments', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
...
router.get('/doctors', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
        const list = await prisma.user.findMany({
            where: { role: client_1.user_role.DOCTOR },
            include: {
                doctorprofile: {
                    include: {
                        department: true,
...
// ADD A NEW DOCTOR
router.post('/doctors', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    ...
    try {
        ...
        const createdUser = await prisma.user.create({
            data: {
                ...
                name,
                phone,
                role: client_1.user_role.DOCTOR,
                status: 'ACTIVE',
                doctorprofile: {
                    create: {
...
// EDIT DOCTOR DETAILS
router.put('/doctors/:id', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
router.put('/doctors/:id/status', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
// 5. DELETE A DOCTOR
router.delete('/doctors/:id', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
router.get('/patients', auth_1.authenticateJWT, async (req, res) => {
    ...
    try {
        const list = await prisma.user.findMany({
            where: { role: client_1.user_role.PATIENT },
            include: {
                patientprofile: true,
...
router.put('/patients/:id/status', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
// 8. GET ALL HOSPITALS
router.get('/hospitals', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
...
// 9. CREATE A HOSPITAL
router.post('/hospitals', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
...
router.put('/hospitals/:id', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
router.delete('/hospitals/:id', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    const { id } = req.params;
...
// 12. GET ALL PAYMENTS
router.get('/payments', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
...
// 13. GET ALL REPORTS
router.get('/reports', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
...
router.post('/notifications', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
...
router.patch('/settings', auth_1.authenticateJWT, async (req, res) => {
    if (!req.user || req.user.role !== 'ADMIN') {
        return res.status(403).json({ message: 'Admin access required' });
    }
    try {
