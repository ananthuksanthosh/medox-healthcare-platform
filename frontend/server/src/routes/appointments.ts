...
import { PrismaClient, appointment_status } from '@prisma/client';
  ...
  try {
    ...
    const appointment = await prisma.appointment.create({
      data: {
        ...
        tokenNumber,
        reason,
        status: appointment_status.CONFIRMED // Automatically confirm on successful payment simulation
      }
    });
...
router.put('/:id/complete', authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
...
router.put('/:id/reschedule', authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
